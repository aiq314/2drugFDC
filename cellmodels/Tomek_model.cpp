...
void Tomek_model::___printCelltype(int celltype)
{
...
void Tomek_model::___applyDrugEffect(double conc, double *ic50, double epsilon)
{
...
/* Added by ALI */
/*==============*/
void Tomek_model::___applyDrugCombination(const double conc_a, const double conc_b, double *ic50_a, double *ic50_b, double ddis_la_effect, bool is_ddis_la, double epsilon)
{
 /* For now, let's assume that Bliss Independence model is applied to all channels */  
 double ea[7], eb[7], ecomb_bi[7];
 for(int i=0;i<7;i++)
 {
  ...
  ecomb_bi[i] = ea[i] + eb[i] - ea[i] * eb[i];
 }
 /* For PCa channel */
 if(ic50_a[0] > epsilon && ic50_a[1] > epsilon && 
    ic50_b[0] > epsilon && ic50_b[1] > epsilon)
 {
  CONSTANTS[PCa] *= (1.0 - ecomb_bi[0]); // Remaining current
 }
...
void Tomek_model::___initConsts()
{
CONSTANTS[nao] = 140.0;
CONSTANTS[cao] = 1.8;
CONSTANTS[ko] = 5.0;
...
void Tomek_model::initConsts (double type, double conc, double *ic50)
{
___printCelltype((int)type);
...
void Tomek_model::initConsts (double celltype, double conc_a, double conc_b, double *ic50_a, double *ic50_b, double ddis_la_effect, bool is_dutta, bool is_ddis_la)
{
CONSTANTS[0] = celltype;
double epsilon = 10E-14;
___initConsts();
if(conc_a > epsilon && conc_b > epsilon){
 ___applyDrugCombination(conc_a,conc_b,ic50_a, ic50_b, ddis_la_effect, is_ddis_la, epsilon);
